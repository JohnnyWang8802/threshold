...
- **按时间自动切换亮 / 暗** — 06:00–18:00 是白天，其余是夜晚；没有手动开关，也不看系统深浅色偏好
...
因为原图也被嵌进去了（缩到 `SEED_MAX=1100`、编码成 JPEG）。早期版本只预埋了算好的网格，
没有原图像素，这两个滑杆在首次打开时永远是灰的——访客点开在线链接看到的第一印象就是残缺的。
现在 `export_grid.py` 会顺带塞一份瘦身版原图，网页应用拿它走跟真实上传完全相同的 `prepped()` 路径。
